...
class ExportProjectDialog extends StatefulWidget {
  ...
  const ExportProjectDialog({super.key, required this.rootNode});
...
class _ExportProjectDialogState extends State<ExportProjectDialog> {
  ...
  Future<void> _saveAsPdf(
    ...
    final double availableHeight =
        pageHeight - contentStartY - marginBottom - footerHeight - footerGap;

    final int linesPerPage = math.max(
      1,
    ...
    for (int i = 0; i < lines.length; i += linesPerPage) {
      final int end = math.min(i + linesPerPage, lines.length);
    ...
    for (int pageIndex = 0; pageIndex < pages.length; pageIndex++) {
      ...
      final double footerY = pageHeight - marginBottom - footerHeight + 4;

      // Footer divider.
      page.graphics.drawLine(
        PdfPen(PdfColor(224, 228, 234), width: 0.8),
        Offset(marginLeft, footerY),
        Offset(pageWidth - marginRight, footerY),
      );
      ...
      page.graphics.drawString(
        ...
        regularFont,
        bounds: Rect.fromLTWH(marginLeft, footerY + 6, contentWidth, 14),
        format: PdfStringFormat(alignment: PdfTextAlignment.center),
        brush: PdfSolidBrush(PdfColor(80, 88, 100)),
      );
    }
    ...
    final String finalPath = '$directory${Platform.pathSeparator}$fileName';
  ...
  Future<void> _handleStartGeneration() async {
    ...
    try {
      if (_format != 'clipboard') {
        targetDirectory ??= await FilePicker.getDirectoryPath();
        ...
        _selectedTargetDirectory = targetDirectory;
      }

      String? fileName;

      if (_format == 'txt' || _format == 'pdf') {
        final String suggestedName = widget.rootNode.name.trim().isEmpty
            ? 'project_export'
        ...
        fileName = await _askForFileName(
          ...
          title: _format == 'pdf' ? 'Name your PDF' : 'Name your text file',
        );
      ...
      if (_format == 'clipboard') {
        progressMessage.value = 'Generating project export...';

        final content = await _generateContent(progressMessage);

        progressMessage.value = 'Copying export to clipboard...';

        final success = await ClipboardService.copyToClipboard(content);

        progressMessage.dispose();

        if (!success) {
          throw Exception('The export could not be copied to the clipboard.');
        }

        if (mounted) {
          Navigator.of(context).pop();
          _showCustomSnackBar(context, 'Project export copied to clipboard!');
        }
      ...
      final progressDialogFuture = _showProgressDialog(progressMessage);
      ...
      if (_format == 'pdf') {
        progressMessage.value = 'Creating your PDF...';

        await _saveAsPdf(targetDirectory!, fileName!, content, progressMessage);
      } else {
        progressMessage.value = 'Saving your text file...';

        final finalPath = '$targetDirectory${Platform.pathSeparator}$fileName';
      ...
      if (_format == 'pdf' || _format == 'txt') {
        ...
        await _showSuccessDialog(filePath: finalPath, format: _format);
      }
    } catch (e) {
      if (mounted) {
        setState(() {
  ...
  Future<String> _generateContent(ValueNotifier<String> progressMessage) async {
    progressMessage.value = 'Generating project structure...';
    ...
    if (_exportMode == 2) {
      return ExportService.exportProjectContent(
        ...
        onProgress: (path) {
          ...
          progressMessage.value = 'Reading: $fileName';
        },
      );
    }

    return ExportService.exportTreeContent(widget.rootNode, options);
  }
  ...
  Widget _buildTooltip({required String message}) {
    return Tooltip(
      message: message,
  ...
  Widget _buildLayoutCard({
    ...
    final selected = _contentsLayout == value;
      ...
      child: Tooltip(
        ...
        child: InkWell(
          ...
          child: AnimatedContainer(
            ...
            decoration: BoxDecoration(
              color: selected ? Colors.white : Colors.white.withOpacity(0.96),
              border: Border.all(
            ...
            child: Row(
              ...
              children: [
                Radio<int>(
                  value: value,
                  groupValue: _contentsLayout,
                  activeColor: AppColors.primary,
                  onChanged: (val) {
                    setState(() {
                      _contentsLayout = val ?? value;
                    });
                  },
                ),
  ...
  Widget build(BuildContext context) {
    return AlertDialog(
      ...
      title: const Row(
        children: [
          Icon(Icons.file_download_rounded, color: AppColors.primary, size: 23),
          SizedBox(width: 10),
          Text(
              ...
              Wrap(
                ...
                children: [
                  ...
                  _buildFormatChip(
                    ...
                    label: 'TXT',
                    tooltip: 'Save the export as a normal plain-text file.',
                  ),
                  _buildFormatChip(
                    value: 'pdf',
                    icon: Icons.picture_as_pdf_outlined,
                    label: 'PDF',
                    tooltip: 'Create a polished, easy-to-read PDF document.',
                  ),
                ],
              ),
                ...
                Container(
                  ...
                  child: Row(
                    children: [
                      ...
                      Expanded(
                        child: Text(
                          _selectedTargetDirectory ?? 'No folder selected yet',
                          maxLines: 2,
                          overflow: TextOverflow.ellipsis,
                          style: const TextStyle(
                            color: AppColors.textSecondary,
      ...
      actions: [
        ...
        FilledButton.icon(
          ...
          onPressed: _isProcessing ? null : _handleStartGeneration,
