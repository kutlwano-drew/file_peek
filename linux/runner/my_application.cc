...
// Set the application/window icon from the bundled Flutter asset.

static void set_application_icon(GtkWindow* window) {

  gchar* executable_path = g_file_read_link(
      "/proc/self/exe",
      nullptr
  );
  ...
  if (executable_path == nullptr) {
    g_warning(
        "Could not determine File Peek executable path."
    );

    return;
  }

  gchar* executable_dir = g_path_get_dirname(
      executable_path
  );
  ...
  if (g_file_test(icon_path, G_FILE_TEST_EXISTS)) {
    ...
    if (error != nullptr) {
      ...
      g_warning(
          "Failed to load File Peek icon: %s",
          error->message
      );
...
static void my_application_activate(GApplication* application) {
  ...
  // Use the File Peek icon instead of the default Flutter icon.
